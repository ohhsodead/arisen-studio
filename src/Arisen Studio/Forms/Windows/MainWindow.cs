...
namespace ArisenStudio.Forms.Windows
{
    public partial class MainWindow : RibbonForm
    {
        ...
        /// </summary>
        public static DatabaseClient Database { get; private set; }
        ...
        /// </summary>
        public static AsyncFtpClient FtpClient { get; private set; }
        ...
        /// Get the 
        /// </summary>
        public static System.Windows.Forms.Timer StatusIdleTimer { get; private set; } = new System.Windows.Forms.Timer();
        ...
        {
            overlayBackground = new() { BorderStyle = BorderStyles.NoBorder, Size = Window.WindowState == FormWindowState.Maximized ? new Size(Window.Size.Width - 0, Window.Size.Height - 10) : new Size(Window.Size.Width + 40, Window.Size.Height - 28), Location = new Point(-0, 36), Anchor = AnchorStyles.Top | AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Bottom };
        ...
        private async void MainWindow_Load(object sender, EventArgs e)
        {
            ...
            {
                _ = XtraMessageBox.Show(this, ResourceLanguage.GetString("NO_INTERNET_CONNECTION"), ResourceLanguage.GetString("NO_INTERNET_CONNECTION_TITLE"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        ...
        /// <summary>
        /// Check the server status by trying to read a test file
        /// </summary>
        public async Task<bool> CheckDatabaseStatus()
        {
            ...
            try
            {
                _httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("ArisenStudio-CheckDatabaseStatus");
        ...
        /// <summary>
        /// Load the database files into the app, either from web or cache.
        /// </summary>
        private static async Task LoadDataAsync()
        {
            try
            {
                Database = await DatabaseClient.InitializeAsync();//.ConfigureAwait(true);
            }
            catch (Exception ex)
            {
                Program.Log.Error(ex, $"{ResourceLanguage.GetString("UNABLE_LOAD_DATABASE")} {ResourceLanguage.GetString("ERROR")}: {ex.Message}");
        ...
        private void InitializeFinished()
        {
            ...
            SkinElement element = SkinManager.GetSkinElement(SkinProductId.Editors, UserLookAndFeel.Default, "SplashForm");
            element.Image.SizingMargins.All = 0;
            element.Color.SolidImageCenterColor = Color.FromName("@Paint High");
            LookAndFeelHelper.ForceDefaultLookAndFeelChanged();

            CloseSplashScreenProgress(handleSplashScreenProgress);

            // Idle Status Timer
            StatusIdleTimer.Tick += StatusIdleTimer_Tick;
            StatusIdleTimer.Interval = 15000;
            StatusIdleTimer.Enabled = true;
        ...
        private void SetConsoleProfile()
        {
            ...
            {
                _ = XtraMessageBox.Show(this, ResourceLanguage.GetString("MUST_HAVE_DEFAULT_CONSOLE"), ResourceLanguage.GetString("LABEL_DEFAULT_CONSOLE"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        ...
        private void ButtonHeaderDonate_ItemClick(object sender, ItemClickEventArgs e)
        {
            _ = Process.Start(Urls.DonatePatreon);
        }

        public void UpdateAvailable(bool isNewUpdate)
        {
            ButtonHeaderUpdate.Visibility = isNewUpdate ? BarItemVisibility.Always : BarItemVisibility.Never;
        }
        ...
        private void ButtonHowToGuides_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            {
                _ = Process.Start(Urls.ProjectWebsite);
            }
        }
        ...
        private void ButtonDownloadsFolder_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (!downloadsPath.IsNullOrEmpty())
            {
                ...
                {
                    _ = XtraMessageBox.Show(this, ResourceLanguage.GetString("DIRECTORY_NOT_EXIST"), ResourceLanguage.GetString("DIRECTORY_NOT_FOUND"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
        ...
        private void ButtonOpenLogFolder_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            {
                _ = Directory.CreateDirectory(UserFolders.Logs);
            }

            _ = Process.Start(UserFolders.Logs);
        }
        ...
        {
            _ = Process.Start($"{Urls.GitHubRepo}issues/new/choose");
        }
        ...
        private void ButtonToolsPsGamesOpenGUI_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            _ = gameLauncher.ShowDialog(this);
            SetStatus("webMAN Commands - Successfully Opened Game Launcher.");
        }

        private async void ButtonToolsPsGamesMountBD_ItemClick(object sender, ItemClickEventArgs e)
        {
            SetStatus("Fetching games (BD) list...");

            List<ListItem> games = await FtpExtensions.GetGamesBdAsync();

            switch (games.Count)
            {
                case <= 0:
                    SetStatus("webMAN Commands - No Games (BD) Found.");
                    _ = XtraMessageBox.Show(this, "No games (BD) can be found on your console. If you have them on external devices then enable this option in your settings.", "No Games Found", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
            }
        ...
        private async void ButtonToolsPsGamesMountISO_ItemClick(object sender, ItemClickEventArgs e)
        {
            SetStatus("Fetching games (ISO) list...");

            List<ListItem> games = await FtpExtensions.GetGamesIsoAsync();

            switch (games.Count)
            {
                case <= 0:
                    SetStatus("webMAN Commands - No Games (ISO) Found.");
                    _ = XtraMessageBox.Show(this, "No games (ISO) can be found on your console. If you have them on external devices then enable this option in your settings.", "No Games Found", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
            }
        ...
        private async void ButtonToolsPsGamesMountPSN_ItemClick(object sender, ItemClickEventArgs e)
        {
            SetStatus("Fetching games (PSN) list...");

            List<ListItem> games = await FtpExtensions.GetGamesPsnAsync();

            switch (games.Count)
            {
                case <= 0:
                    SetStatus("webMAN Commands - No Games (PSN) Found.");
                    _ = XtraMessageBox.Show(this, "No games (PSN) can be found on your console. If you have them on external devices then enable this option in your settings.", "No Games Found", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
            }
        ...
        private void ButtonToolsPsVirtualController_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            _ = Process.Start("http://pad.aldostools.org/pad.html");
            SetStatus("webMAN Commands - Virtual Controller: Opened in Web Browser");
        }

        private void ButtonToolsPsScreenshot_ItemClick(object sender, ItemClickEventArgs e)
        {
            DialogExtensions.ShowPs3TakeScreenshot(this);
        }
        ...
        {
            DialogExtensions.ShowXboxTakeScreenshot(this);
        }

        private void ButtonToolsXboxNeighborhoodEditor_ItemClick(object sender, ItemClickEventArgs e)
        {
            DialogExtensions.ShowXboxNeighborhoodEditor(this);
        }
        ...
        private void DisconnectConsole()
        {
            ...
            _ = XtraMessageBox.Show(this, ResourceLanguage.GetString("SUCCESS_DISCONNECTED"), ResourceLanguage.GetString("SUCCESS"), MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private async void NavigationItemPackages_Click(object sender, EventArgs e)
        {
        ...
        private async void NavigationItemGameCheats_ClickAsync(object sender, EventArgs e)
        {
            ...
            {
                await LoadGameCheatsCategories();
                await SearchGameCheats();
        ...
        private void LoadOurFavoriteGames()
        {
            ...
            foreach (Models.Dashboard.FavoriteGamesData.Favorite game in Database.FavoriteGames.Favorites)
            {
                ...
                {
                    _ = TileGroupFavoriteGames.Items.Add(FavoriteModItem(game.GetPlatform(), game.CategoryId, game.ImageUrl));
                }
            }
        ...
        private void LoadOurFavoriteMods()
        {
            ...
            foreach (Models.Dashboard.FavoriteModsData.Favorite favorite in Database.FavoriteMods.Favorites)
            {
                ...
                {
                    _ = TileGroupOurFavoriteMods.Items.Add(FavoriteModItem(favorite.GetPlatform(), favorite.CategoryId, favorite.ModId, favorite.Name));
                }
            }
        }
        ...
        {
            if (WebViewCurrentPoll.CoreWebView2 == null)
            {
                WebViewCurrentPoll.CoreWebView2InitializationCompleted += WebViewPoll_CoreWebView2InitializationCompleted;
                WebViewCurrentPoll.NavigationCompleted += WebViewPoll_NavigationCompleted;
                WebViewCurrentPoll.NavigationStarting += WebViewPoll_NavigationStarting;
                await WebViewCurrentPoll.EnsureCoreWebView2Async(null);
            }
        }
        ...
        private void LoadStatistics()
        {
            ...
            foreach (var category in data)
            {
                ...
                {
                    _ = series.Points.Add(new SeriesPoint($"{category.Key} - {subCategory.Key}", subCategory.Value));
                }
            }

            // Add the series to the chart
            //_ = chartControl1.Series.Add(series);
            ...
            LabelStatsPlayStation3.Text =
                ...
                //$"{Database.GamesDataPS3.Games.Count():N0} {ResourceLanguage.GetString("LABEL_GAME_CHEATS")}";
        ...
        private void TileItemDownloadsOpenFile_ItemClick(object sender, TileItemEventArgs e)
        {
            ...
            {
                _ = Process.Start("explorer.exe", $"/select,\"{downloadedItem.FilePath}\"");
            }
            else
            {
                _ = XtraMessageBox.Show(this, $"{ResourceLanguage.GetString("DIRECTORY_NOT_EXIST")}.\n\nPath: {downloadedItem.FilePath}", ResourceLanguage.GetString("DIRECTORY_NOT_FOUND"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        }
        ...
        private void TileItemDownloadsDeleteItem_ItemClick(object sender, TileItemEventArgs e)
        {
            if (XtraMessageBox.Show(this, ResourceLanguage.GetString("CONFIRM_DELETE_ITEM"), ResourceLanguage.GetString("CONFIRM_DELETE"), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                if (downloadedItem != null)
                {
                    ...
                    _ = Settings.DownloadedMods.RemoveAll(x => x == downloadedItem);
                    SearchDownloads();
                }
            }
        }
        ...
        public void LoadDownloads()
        {
            ...
            foreach (DownloadedItem downloadedItem in Settings.DownloadedMods)
            {
                ...
                if (modItemData != null)
                {
                    _ = DataTableDownloads.Rows.Add(modItemData.Id,
        ...
        private void TileItemInstalledFilesDeleteItem_ItemClick(object sender, TileItemEventArgs e)
        {
            if (GridViewInstalledFiles.SelectedRowsCount > 0)
            {
                if (XtraMessageBox.Show(this, ResourceLanguage.GetString("CONFIRM_DELETE_ITEM"), ResourceLanguage.GetString("CONFIRM_DELETE"), MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) == DialogResult.Yes)
                {
                    ...
                    _ = Settings.ConsoleProfiles.Find(x => x.Platform == platform && x.Id == consoleId).InstalledMods.Remove(installedModInfo);
                    _ = XtraMessageBox.Show(this, ResourceLanguage.GetString("DELETE_ITEM_SUCCESS"), ResourceLanguage.GetString("DELETED"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
            }
        }
                ...
                if (GridViewInstalledFiles.RowCount > 0)
                {
                    ...
                    foreach (ConsoleProfile consoleProfile in Settings.ConsoleProfiles.FindAll(x => x.Id == consoleId))
                    {
                        foreach (InstalledModInfo installedModInfo in consoleProfile.InstalledMods)
                        {
                            ...
                            {
                                _ = XtraMessageBox.Show(this, string.Format(ResourceLanguage.GetString("UNINSTALL_CONNECTION_REQUIREMENT"), consoleProfile.Name, consoleProfile.Platform.Humanize()), ResourceLanguage.GetString("ERROR"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                return;
                            }
                            else if (IsConsoleConnected && ConsoleProfile.Id != consoleProfile.Id)
                            {
                                _ = XtraMessageBox.Show(this, string.Format(ResourceLanguage.GetString("UNINSTALL_CONNECT_TO_REQUIRED_CONSOLE"), consoleProfile.Name, consoleProfile.Platform.Humanize()), ResourceLanguage.GetString("ERROR"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                return;
                            }
        ...
        {
            _ = GridControlFileManagerLocalFiles.Focus();
            ...
            {
                _ = ComboBoxFileManagerLocalDrives.Properties.Items.Add(driveInfo.Name.Replace(@"\", string.Empty));
            }
        ...
        public void LoadLocalDirectory(string directoryPath)
        {
            try
            {
                ...
                {
                    case false:
                        _ = DataTableLocalFiles.Rows.Add("folder",
                                                     ImageFolderUp,
                                                     "..",
                                                     string.Empty,
                                                     string.Empty);
                ...
                foreach (string fileItem in Directory.GetFiles(DirectoryPathLocal))
                {
                    ...
                    _ = DataTableLocalFiles.Rows.Add("file",
                                        ImageFile,
        ...
        private void GridViewFileManagerLocalFiles_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left && downHitInfo != null)
            {
                ...
                {
                    _ = view.GridControl.DoDragDrop(downHitInfo, DragDropEffects.Copy);
                    downHitInfo = null;
                }
            }
        }
        ...
        public void CreateLocalFolder()
        {
            try
            {
                ...
                if (newName != null)
                {
                    ...
                    {
                        _ = XtraMessageBox.Show(ResourceLanguage.GetString("FOLDER_NAME_EXISTS"), ResourceLanguage.GetString("ERROR"), MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    else
                    {
            ...
            catch (Exception ex)
            {
                ...
                _ = XtraMessageBox.Show(string.Format(ResourceLanguage.GetString("FOLDER_CREATE_ERROR"), ex.Message), ResourceLanguage.GetString("ERROR"), MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        public async void UploadLocalFile()
        {
            try
            {
                string type = GridViewFileManagerLocalFiles.GetFocusedRowCellDisplayText(GridViewFileManagerLocalFiles.Columns[0]);
                    ...
                    case "file":
                        {
                            ...
                            if (File.Exists(localPath))
                            {
                                ...
                                {
                                    case Platform.PS3:
                                        _ = await FtpExtensions.UploadFileAsync(localPath, consolePath);
                                        break;
                                    default:
                    ...
                    case "folder":
                        {
                            ...
                            _ = FtpClient.UploadDirectory(localPath, consolePath, FtpFolderSyncMode.Update, FtpRemoteExists.Overwrite);
                        ...
                        case false:
                            {
                                ...
                                switch (type)
                                {
                                    case "folder":
                                        ...
                                        IoExtensions.DeleteDirectory(selectedItem);
        ...
        public async void LoadConsoleDirectory(string directoryPath)
        {
            try
            {
                ...
                if (Platform == Platform.PS3)
                {
                    await FtpClient.SetWorkingDirectory(DirectoryPathConsole);
                    ...
                    foreach (FtpListItem listItem in await FtpClient.GetListing(DirectoryPathConsole))
                    {
        ...
        private void ButtonConsoleAddToModules_Click(object sender, EventArgs e)
        {
            ...
            if (XtraMessageBox.Show(ResourceLanguage.GetString("FILE_RENAME_MODULE"), ResourceLanguage.GetString("FILE_RENAME"), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    _ = XtraMessageBox.Show(ResourceLanguage.GetString("FILE_NAME_EMPTY"), ResourceLanguage.GetString("NO_INPUT"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
            }
            ...
            {
                _ = XtraMessageBox.Show(ResourceLanguage.GetString("FILE_NAME_EXISTS_IN_MODULES"), ResourceLanguage.GetString("FILE_NAME_DUPLICATE"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
                    ...
                    case "file":
                        {
                            ...
                            switch (Platform)
                            {
                                case Platform.PS3:
                                    ...
                                    _ = FtpClient.DownloadFile(localFile, consoleFile, FtpLocalExists.Overwrite);
                                    break;
                                case Platform.XBOX360:
                                    if (File.Exists(localFile))
                                    {
                                ...
                                case Platform.PS3:
                                    {
                                        ...
                                        {
                                            IoExtensions.DeleteDirectory(localPath);
                                        }
        ...
        private async Task RenameConsoleFile()
        {
            try
            {
                ...
                if (newFileName != null && !newFileName.Equals(oldFileName))
                {
                    ...
                    {
                        case Platform.PS3:
                            if (await FtpClient.FileExists(newConsoleFilePath))
                            {
        ...
        private async void RenameConsoleFolder()
        {
            try
            {
                ...
                if (newFolderName != null && !newFolderName.Equals(oldFolderName))
                {
                    if (await FtpClient.DirectoryExists(newFolderPath))
                    {
        ...
        public async void CreateConsoleFolder()
        {
            try
            {
                ...
                if (folderName != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            case Platform.PS3:
                                _ = FtpClient.CreateDirectory(folderPath, true);
                                break;
                            case Platform.XBOX360:
        ...
        private void LoadLanguages()
        {
            ...
            {
                _ = ComboBoxSettingsLanguages.Properties.Items.Add(language.Key);
            }
        ...
        private void ChangeLanguage(string languageName)
        {
            try
            {
                ...
                {
                    _ = XtraMessageBox.Show(this, ResourceLanguage.GetString("LANGUAGE_HELP"), ResourceLanguage.GetString("LANGUAGE_NOT_SUPPORTED"), MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                    ComboBoxSettingsLanguages.SelectedItem = CurrentLanguage;
                    return;
                }
                else
            ...
            try
            {
                ...
                ButtonToolsXboxNeighborhoodEditor.Caption = ResourceLanguage.GetString("NEIGHBORHOOD_EDITOR");
                ...
                LabelSettingsPathTemp.Text = ResourceLanguage.GetString("LABEL_TEMP_FOLDER");
                ...
                _ = ComboBoxGameModsPS3FilterStatus.Properties.Items.Add("<" + ResourceLanguage.GetString("LABEL_ALL") + ">");
                _ = ComboBoxGameModsPS3FilterStatus.Properties.Items.Add(ResourceLanguage.GetString("LABEL_NOT_INSTALLED"));
                _ = ComboBoxGameModsPS3FilterStatus.Properties.Items.Add(ResourceLanguage.GetString("LABEL_INSTALLED"));
                ...
                _ = ComboBoxHomebrewFilterStatus.Properties.Items.Add("<" + ResourceLanguage.GetString("LABEL_ALL") + ">");
                _ = ComboBoxHomebrewFilterStatus.Properties.Items.Add(ResourceLanguage.GetString("LABEL_NOT_INSTALLED"));
                _ = ComboBoxHomebrewFilterStatus.Properties.Items.Add(ResourceLanguage.GetString("LABEL_INSTALLED"));
                ...
                _ = ComboBoxResourcesFilterStatus.Properties.Items.Add("<" + ResourceLanguage.GetString("LABEL_ALL") + ">");
                _ = ComboBoxResourcesFilterStatus.Properties.Items.Add(ResourceLanguage.GetString("LABEL_NOT_INSTALLED"));
                _ = ComboBoxResourcesFilterStatus.Properties.Items.Add(ResourceLanguage.GetString("LABEL_INSTALLED"));
                ...
                _ = ComboBoxPackagesFilterStatus.Properties.Items.Add("<" + ResourceLanguage.GetString("LABEL_ALL") + ">");
                _ = ComboBoxPackagesFilterStatus.Properties.Items.Add(ResourceLanguage.GetString("LABEL_NOT_INSTALLED"));
                _ = ComboBoxPackagesFilterStatus.Properties.Items.Add(ResourceLanguage.GetString("LABEL_INSTALLED"));
                ...
                _ = ComboBoxGameModsXboxFilterStatus.Properties.Items.Add("<" + ResourceLanguage.GetString("LABEL_ALL") + ">");
                _ = ComboBoxGameModsXboxFilterStatus.Properties.Items.Add(ResourceLanguage.GetString("LABEL_NOT_INSTALLED"));
                _ = ComboBoxGameModsXboxFilterStatus.Properties.Items.Add(ResourceLanguage.GetString("LABEL_INSTALLED"));
                ...
                _ = ComboBoxHomebrewPS4FilterStatus.Properties.Items.Add("<" + ResourceLanguage.GetString("LABEL_ALL") + ">");
                _ = ComboBoxHomebrewPS4FilterStatus.Properties.Items.Add(ResourceLanguage.GetString("LABEL_NOT_INSTALLED"));
                _ = ComboBoxHomebrewPS4FilterStatus.Properties.Items.Add(ResourceLanguage.GetString("LABEL_INSTALLED"));
        ...
        {
            _ = Process.Start("https://crowdin.com/project/arisenstudio");
        }
        ...
        {
            string path = DialogExtensions.ShowFolderBrowseDialog(this, ResourceLanguage.GetString("LABEL_TEMP_FOLDER"));
        ...
        private void TileItemCustomDelete_ItemClick(object sender, TileItemEventArgs e)
        {
            if (SelectedCustomModId != -1)
            {
                ...
                {
                    _ = Settings.CustomMods.Remove(customItem);
                    _ = XtraMessageBox.Show(this, ResourceLanguage.GetString("DELETE_ITEM_SUCCESS"), ResourceLanguage.GetString("SUCCESS"), MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
        ...
        private void LoadCustomModsCategories()
        {
            ...
            _ = ComboBoxCustomModsFilterCategory.Properties.Items.Add($"<{ResourceLanguage.GetString("ALL_CATEGORIES")}>");

            ComboBoxCustomModsFilterModType.Properties.Items.Clear();
            _ = ComboBoxCustomModsFilterModType.Properties.Items.Add($"<{ResourceLanguage.GetString("ANY")}>");
            ...
            foreach (CategoryItem category in Database.CategoriesData.Categories.OrderBy(x => x.Title))
            {
                ...
                {
                    _ = ComboBoxCustomModsFilterCategory.Properties.Items.Add(category.Title);
                }
            }
            ...
            foreach (CustomItemData customMod in Settings.CustomMods)
            {
                foreach (string modType in from string modType in customMod.ModType
                                           ...
                                           select modType)
                {
                    _ = ComboBoxCustomModsFilterModType.Properties.Items.Add(modType);
                }
                ...
                foreach (string version in from string version in customMod.Version
                                           ...
                                           select version)
                {
                    _ = ComboBoxCustomModsFilterVersion.Properties.Items.Add(version);
                }
            }
        }
        ...
        private void SearchCustomMods()
        {
            ...
            _ = BeginInvoke(new Action(() =>
            {
                ...
                x.Version.EqualsIgnoreCase(FilterCustomModsVersion)))
                {
                    ...
                    _ = DataTableCustomMods.Rows.Add(
                        customMod.Id,
        ...
        private void LoadGameModsCategoriesPS3()
        {
            ...
            _ = ComboBoxGameModsPS3FilterGame.Properties.Items.Add($"<{ResourceLanguage.GetString("ALL_CATEGORIES")}>");

            ComboBoxGameModsPS3FilterModType.Properties.Items.Clear();
            _ = ComboBoxGameModsPS3FilterModType.Properties.Items.Add($"<{ResourceLanguage.GetString("ANY")}>");

            ComboBoxGameModsPS3FilterRegion.Properties.Items.Clear();
            _ = ComboBoxGameModsPS3FilterRegion.Properties.Items.Add($"<{ResourceLanguage.GetString("ANY")}>");
            ...
            foreach (CategoryItem category in Database.CategoriesData.Categories.FindAll(x => CategoryType.Game == x.CategoryType).OrderBy(x => x.Title))
            {
                ...
                {
                    _ = ComboBoxGameModsPS3FilterGame.Properties.Items.Add(category.Title);
                }
            }
            ...
            foreach (ModItemData modItemData in Database.GameModsPS3.Library)
            {
                foreach (string modType in from string modType in modItemData.ModTypes
                                           ...
                                           select modType)
                {
                    _ = ComboBoxGameModsPS3FilterModType.Properties.Items.Add(modType);
                }
                ...
                foreach (string region in from string region in modItemData.Regions
                                          ...
                                          select region)
                {
                    _ = ComboBoxGameModsPS3FilterRegion.Properties.Items.Add(region);
                }
                ...
                foreach (string version in from string version in modItemData.Versions
                                           ...
                                           select version)
                {
                    _ = ComboBoxGameModsPS3FilterVersion.Properties.Items.Add(version);
                }
            }
        }
        ...
        private void SearchGameModsPS3()
        {
            ...
            _ = BeginInvoke(new Action(() =>
            {
                foreach (ModItemData modItemData in Database.GameModsPS3.GetGameMods(Database.CategoriesData, FilterGameModsPS3CategoryId, FilterGameModsPS3Name, FilterGameModsPS3SystemType, FilterGameModsPS3Type, FilterGameModsPS3Region, FilterGameModsPS3Version, FilterGameModsPS3ShowFavorites))
                {
                    ...
                    _ = DataTableGameModsPS3.Rows.Add(modItemData.Id,
        ...
        private void GridViewGameModsPS3_RowClick(object sender, RowClickEventArgs e)
        {
            ...
            if (info.InRow)
            {
                SelectedGameModPS3Id = (int)GridViewGameModsPS3.GetRowCellValue(info.RowHandle, GridViewGameModsPS3.Columns[0]);
            }

            TileItemGameModsPS3AddFavorite.Enabled = SelectedGameModPS3Id != -1;
            TileItemGameModsPS3Download.Enabled = SelectedGameModPS3Id != -1;
            TileItemGameModsPS3ShowDetails.Enabled = SelectedGameModPS3Id != -1;
        }
        ...
        private int SelectedHomebrewPS3Id = 0;
        ...
        {
            if (SelectedHomebrewPS3Id != -1)
            {
                var modItem = Database.HomebrewPS3.GetModById(Platform.PS3, SelectedHomebrewPS3Id);
        ...
        private void TileItemHomebrewShowDetails_ItemClick(object sender, TileItemEventArgs e)
        {
            if (SelectedHomebrewPS3Id != -1)
            {
                ShowHomebrewDetails(Platform.PS3, SelectedHomebrewPS3Id);
            }
        }
        ...
        private void LoadHomebrewCategoriesPS3()
        {
            ...
            foreach (CategoryItem category in categories.OrderBy(x => x.Title))
            {
                ...
                {
                    if (!ComboBoxHomebrewFilterCategory.Properties.Items.Contains(category.Title))
                    {
                        _ = ComboBoxHomebrewFilterCategory.Properties.Items.Add(category.Title);
                    }
                }
            }
            ...
            _ = ComboBoxHomebrewFilterVersion.Properties.Items.Add($"<{ResourceLanguage.GetString("ANY")}>");
            ...
            foreach (ModItemData modItemData in Database.HomebrewPS3.Library)
            {
                foreach (string version in from string version in modItemData.Versions
                                           ...
                                           select version)
                {
                    _ = ComboBoxHomebrewFilterVersion.Properties.Items.Add(version);
                }
            }
        }
        ...
        private void SearchHomebrewPS3()
        {
            ...
            _ = BeginInvoke(new Action(() =>
            {
                foreach (ModItemData modItemData in Database.HomebrewPS3.GetHomebrew(Database.CategoriesData, FilterHomebrewCategoryId, FilterHomebrewName, FilterHomebrewSystemType, FilterHomebrewVersion, FilterHomebrewShowFavorites))
                {
                    ...
                    _ = DataTableHomebrew.Rows.Add(modItemData.Id,
                                               modItemData.GetCategoryName(Database.CategoriesData),
                                               modItemData.Name,
                                               modItemData.FirmwareType,
                                               string.Join(" & ", modItemData.Versions),
        ...
        private void GridViewHomebrewPS3_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            if (GridViewHomebrewPS3.RowCount > 0)
            {
                if (e.FocusedRowHandle != -1)
                {
                    SelectedHomebrewPS3Id = (int)GridViewHomebrewPS3.GetRowCellValue(e.FocusedRowHandle, GridViewHomebrewPS3.Columns[0]);
                }
            }

            TileItemHomebrewAddFavorite.Enabled = e.FocusedRowHandle != -1;
            TileItemHomebrewDownload.Enabled = e.FocusedRowHandle != -1;
            TileItemHomebrewShowDetails.Enabled = e.FocusedRowHandle != -1;
        }

        private void GridViewHomebrewPS3_RowClick(object sender, RowClickEventArgs e)
        {
            DXMouseEventArgs ea = e;
            GridView view = sender as GridView;
            GridHitInfo info = view.CalcHitInfo(ea.Location);
            if (info.InRow)
            {
                SelectedHomebrewPS3Id = (int)GridViewHomebrewPS3.GetRowCellValue(info.RowHandle, GridViewHomebrewPS3.Columns[0]);
            }
        }

        private void GridViewHomebrewPS3_DoubleClick(object sender, EventArgs e)
        {
            DXMouseEventArgs ea = e as DXMouseEventArgs;
            GridView view = sender as GridView;
            GridHitInfo info = view.CalcHitInfo(ea.Location);
        ...
        private void LoadResourcesCategories()
        {
            ...
            _ = ComboBoxResourcesFilterModType.Properties.Items.Add($"<{ResourceLanguage.GetString("ANY")}>");

            ComboBoxResourcesFilterVersion.Properties.Items.Clear();
            _ = ComboBoxResourcesFilterVersion.Properties.Items.Add($"<{ResourceLanguage.GetString("ANY")}>");
            ...
            foreach (ModItemData modItemData in Database.ResourcesPS3.Library)
            {
                foreach (string modType in from string modType in modItemData.ModTypes
                                           ...
                                           select modType)
                {
                    _ = ComboBoxResourcesFilterModType.Properties.Items.Add(modType);
                }
                ...
                foreach (string version in from string version in modItemData.Versions
                                           ...
                                           select version)
                {
                    _ = ComboBoxResourcesFilterVersion.Properties.Items.Add(version);
                }
            }
        }
        ...
        private void SearchResources()
        {
            ...
            _ = BeginInvoke(new Action(() =>
            {
                foreach (ModItemData modItemData in Database.ResourcesPS3.GetResources(Database.CategoriesData, FilterResourcesCategoryId, FilterResourcesName, FilterResourcesSystemType, FilterResourcesModType, FilterResourcesVersion, FilterResourcesShowFavorites).OrderBy(x => x.Name))
                {
                    ...
                    _ = DataTableResources.Rows.Add(modItemData.Id,
                                                modItemData.GetCategoryName(Database.CategoriesData),
                                                modItemData.Name,
                                                modItemData.FirmwareType,
                                                modItemData.ModType,
        ...
        private void GridViewResources_RowClick(object sender, RowClickEventArgs e)
        {
            ...
            TileItemResourcesShowFavorites.Enabled = SelectedResourceId != -1;
        ...
        private async void TileItemPackagesSortBy_ItemClick(object sender, TileItemEventArgs e)
        {
        ...
        private async void ComboBoxPackagesFilterCategories_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private async void TextBoxPackagesFilterTitle_EditValueChanged(object sender, EventArgs e)
        {
        ...
        private async void ComboBoxPackagesFilterModifiedDate_EditValueChanged(object sender, EventArgs e)
        {
        ...
        private async void ComboBoxPackagesFilterRegion_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private async void ComboBoxPackagesFilterStatus_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        //                if (shouldLoad)
        //                {
        //                    _ = DataTablePackages.Rows.Add(package.Url,
        //                                               package.Category,
        //                                               package.Name,
        //                                               package.TitleId,
        ...
        {
            // Show loading panel (UI thread)
            GridViewPackages.ShowLoadingPanel();

            // Clear the current data in the grid (UI thread)
            DataTablePackages.Rows.Clear();

            List<PackageItemData> packages;

            // Run the package retrieval logic in the background
            ...
            // Process the packages in the background thread and store the results
            var rowsToAdd = await Task.Run(() =>
            {
                ...
                x.Region.ContainsIgnoreCase(FilterPackagesRegion)).OrderBy(x => x.Name)
                                                                        let isInstalled = Settings.InstalledPackages.Any(installed => installed.Url.Equals(package.Url))
                                                                        select (package, isInstalled))
                {
                    ...
                    if (!package.IsDateMissing)
                    {
                        if (FilterPackagesModifiedDate != null)
                        {
                            shouldLoad = FilterPackagesModifiedDateType switch
                            {
                                FilterType.Equal => DateTime.Parse(package.ModifiedDate) == FilterPackagesModifiedDate,
                                FilterType.MoreThanOrEqual => DateTime.Parse(package.ModifiedDate) >= FilterPackagesModifiedDate,
                                FilterType.LessThanOrEqual => DateTime.Parse(package.ModifiedDate) <= FilterPackagesModifiedDate,
                                _ => shouldLoad
                            };
                        }
                    }
                    else if (FilterPackagesModifiedDate != null)
                    {
                        shouldLoad = false;
                        continue;
                    }
                    ...
                    if (!package.IsSizeMissing)
                    {
                        if (FilterPackagesFileSize != -1)
                        {
                            shouldLoad = FilterPackagesFileSizeType switch
                            {
                                FilterType.Equal => int.Parse(package.Size) == FilterPackagesFileSize,
                                FilterType.MoreThanOrEqual => int.Parse(package.Size) >= FilterPackagesFileSize,
                                FilterType.LessThanOrEqual => int.Parse(package.Size) <= FilterPackagesFileSize,
                                _ => shouldLoad
                            };
                        }
                    }
                    else if (FilterPackagesFileSize != -1)
                    {
                        shouldLoad = false;
                        continue;
                    }

                    // Collect rows to add later (on UI thread)
                    if (shouldLoad)
                    {
            ...
            // Add rows to the DataTable on the UI thread
            foreach (var row in rowsToAdd)
            {
                DataTablePackages.Rows.Add(row);
            }

            // Bind the data to the GridControl (UI thread)
            GridControlPackages.DataSource = DataTablePackages;

            // Set grid column properties (UI thread)
            GridViewPackages.Columns[0].Visible = false;

            GridViewPackages.Columns[1].MinWidth = 98;
            GridViewPackages.Columns[1].MaxWidth = 98;

            GridViewPackages.Columns[3].MaxWidth = 110;
            GridViewPackages.Columns[3].MinWidth = 110;

            GridViewPackages.Columns[4].MaxWidth = 128;
            GridViewPackages.Columns[4].MinWidth = 128;

            GridViewPackages.Columns[5].MaxWidth = 116;
            GridViewPackages.Columns[5].MinWidth = 116;

            GridViewPackages.Columns[6].MaxWidth = 96;
            GridViewPackages.Columns[6].MinWidth = 96;
            ...
            // Hide the loading panel (UI thread)
            GridViewPackages.HideLoadingPanel();
            ...
            // Move the focus to the first row (UI thread)
            GridViewPackages.MoveFirst();
        }
        ...
        private void TileItemGameModsXboxShowDetails_ItemClick(object sender, TileItemEventArgs e)
        {
            if (SelectedGameModXboxId != -1)
            {
                ShowGameModDetails(Platform.XBOX360, SelectedGameModXboxId);
            }
        }
        ...
        private void LoadGameModsCategoriesXbox()
        {
            ...
            _ = ComboBoxGameModsXboxFilterGame.Properties.Items.Add($"<{ResourceLanguage.GetString("ALL_CATEGORIES")}>");
            ...
            foreach (CategoryItem category in Database.CategoriesData.Categories.FindAll(x => CategoryType.Game == x.CategoryType).OrderBy(x => x.Title))
            {
                ...
                {
                    _ = ComboBoxGameModsXboxFilterGame.Properties.Items.Add(category.Title);
                }
            }
            ...
            foreach (ModItemData modItemData in Database.GameModsX360.Library)
            {
                foreach (string version in from string version in modItemData.Versions
                                           ...
                                           select version)
                {
                    _ = ComboBoxGameModsXboxFilterVersion.Properties.Items.Add(version);
                }
            }
        }
        ...
        private void GridViewGameModsXbox_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (info.InRow)
            {
                ...
                ShowGameModDetails(Platform.XBOX360, selectedPlugin.Id);
            }
        }
        ...
        private void LoadHomebrewCategoriesXbox()
        {
            ...
            _ = ComboBoxHomebrewXboxFilterCategory.Properties.Items.Add($"<{ResourceLanguage.GetString("ALL_CATEGORIES")}>");
            ...
            foreach (CategoryItem category in Database.CategoriesData.Categories.FindAll(x => CategoryType.Game == x.CategoryType).OrderBy(x => x.Title))
            {
                ...
                {
                    _ = ComboBoxHomebrewXboxFilterCategory.Properties.Items.Add(category.Title);
                }
            }
            ...
            foreach (ModItemData modItemData in Database.HomebrewX360.Library)
            {
                foreach (string version in from string version in modItemData.Versions
                                           ...
                                           select version)
                {
                    _ = ComboBoxHomebrewXboxFilterVersion.Properties.Items.Add(version);
                }
            }
        }
        ...
        private void LoadHomebrewCategoriesPS4()
        {
            ...
            _ = ComboBoxHomebrewPS4FilterCategory.Properties.Items.Add($"<{ResourceLanguage.GetString("ALL_CATEGORIES")}>");

            ComboBoxHomebrewPS4FilterFwVersion.Properties.Items.Clear();
            _ = ComboBoxHomebrewPS4FilterFwVersion.Properties.Items.Add($"<{ResourceLanguage.GetString("ANY")}>");
            ...
            foreach (CategoryItem category in Database.CategoriesData.Categories.FindAll(x => CategoryType.Homebrew == x.CategoryType).OrderBy(x => x.Title))
            {
                ...
                {
                    _ = ComboBoxHomebrewPS4FilterCategory.Properties.Items.Add(category.Title);
                }
            }
            ...
            foreach (AppItemData appItemData in Database.HomebrewPS4.Library)
            {
                foreach (string fwVersion in from string fwVersion in appItemData.FirmwareVersions
                                             ...
                                             select fwVersion)
                {
                    _ = ComboBoxHomebrewPS4FilterFwVersion.Properties.Items.Add(fwVersion);
                }
                ...
                foreach (string version in from string version in appItemData.Versions
                                           ...
                                           select version)
                {
                    _ = ComboBoxHomebrewPS4FilterVersion.Properties.Items.Add(version);
                }
            }
        }
        ...
        public void SearchHomebrewPS4()
        {
            ...
            foreach (AppItemData appItemData in Database.HomebrewPS4.GetHomebrew(Database.CategoriesData, FilterHomebrewPS4CategoryId, FilterHomebrewPS4Name, FilterHomebrewPS4Version, FilterHomebrewPS4FWVersion, FilterHomebrewPS4ShowFavorites))
            {
                ...
                _ = DataTableHomebrewPS4.Rows.Add(appItemData.Id,
        ...
        private void LoadGamesCategories()
        {
            ...
            _ = ComboBoxGamesFilterCategory.Properties.Items.Add($"<{ResourceLanguage.GetString("ALL_CATEGORIES")}>");

            ComboBoxGamesFilterFwVersion.Properties.Items.Clear();
            _ = ComboBoxGamesFilterFwVersion.Properties.Items.Add($"<{ResourceLanguage.GetString("ANY")}>");
            ...
            foreach (CategoryItem category in Database.CategoriesData.Categories.FindAll(x => CategoryType.Game == x.CategoryType).OrderBy(x => x.Title))
            {
                if (Database.HomebrewPS4.Library.Any(x => x.CategoryId.Equals(category.Id)))
                {
                    _ = ComboBoxGamesFilterCategory.Properties.Items.Add(category.Title);
                }
            }
            ...
            foreach (AppItemData appItemData in Database.HomebrewPS4.Library)
            {
                foreach (string fwVersion in from string fwVersion in appItemData.FirmwareVersions
                                             ...
                                             select fwVersion)
                {
                    _ = ComboBoxGamesFilterFwVersion.Properties.Items.Add(fwVersion);
                }
                ...
                foreach (string version in from string version in appItemData.Versions
                                           ...
                                           select version)
                {
                    _ = ComboBoxGamesFilterVersion.Properties.Items.Add(version);
                }
            }
        }
        ...
        private void LoadGameSavesCategories()
        {
            ...
            foreach (CategoryItem category in Database.CategoriesData.Categories.FindAll(x => x.CategoryType == CategoryType.Game).OrderBy(x => x.Title))
            {
                ...
                {
                    _ = ComboBoxGameSavesFilterCategory.Properties.Items.Add(category.Title);
                }
            }
            ...
            foreach (GameSaveItemData gameSaveItem in Database.GameSaves.GameSaves)
            {
                foreach (string region in from string region in gameSaveItem.Region.Split(' ')
                                          ...
                                          select region)
                {
                    _ = ComboBoxGameSavesFilterRegion.Properties.Items.Add(region);
                }
                ...
                foreach (string version in from string version in gameSaveItem.Version.Split('/')
                                           ...
                                           select version)
                {
                    _ = ComboBoxGameSavesFilterVersion.Properties.Items.Add(version);
                }
            }
        }
        ...
        private void GridViewGameSaves_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            if (e.FocusedRowHandle != -1)
            {
                SelectedGameSaveItem = Database.GameSaves.GetModById(Platform, (int)GridViewGameSaves.GetRowCellValue(e.FocusedRowHandle, GridViewGameSaves.Columns[0]));
            }
        ...
        private void GridViewGameSaves_RowClick(object sender, RowClickEventArgs e)
        {
            ...
            if (info.InRow)
            {
                SelectedGameSaveItem = Database.GameSaves.GetModById(Platform, (int)GridViewGameSaves.GetRowCellValue(e.RowHandle, GridViewGameSaves.Columns[0]));
            }
        ...
        private void GridViewGameSaves_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (info.InRow)
            {
                ...
                //int modId = (int)GridViewGameSaves.GetRowCellValue(info.RowHandle, GridViewGameSaves.Columns[0]);
                //SelectedGameModXboxId = modId;

                SelectedGameSaveItem = Database.GameSaves.GetModById(Platform, (int)GridViewGameSaves.GetRowCellValue(info.RowHandle, GridViewGameSaves.Columns[0]));
        ...
        private void TileItemGameCheatsShowDetails_ItemClick(object sender, TileItemEventArgs e)
        {
            if (SelectedGameSaveItem != null)
            {
                ...
                ShowGameCheats(SelectedGameItem);
            }
        }
        ...
        private async void ComboBoxGameCheatsFilterGame_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ComboBoxGameCheatsFilterGame.SelectedIndex == 0 || ComboBoxGameCheatsFilterGame.SelectedIndex == -1)
            {
                FilterGameCheatsName = string.Empty;
            }
            else
            {
                string selectedCategory = ComboBoxGameCheatsFilterGame.SelectedItem as string;
                FilterGameCheatsName = selectedCategory;
            }

            await SearchGameCheats();
        }

        private async void ComboBoxGameCheatsFilterRegion_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private async void ComboBoxGameCheatsFilterVersion_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        public async Task LoadGameCheatsCategories()
        {
            ComboBoxGameCheatsFilterGame.Properties.Items.Clear();

            _ = ComboBoxGameCheatsFilterGame.Properties.Items.Add($"<{ResourceLanguage.GetString("ALL_GAMES")}>");

            await Task.Run(() =>
            {
                var games = Database.GamesDataPS3.Games.OrderBy(x => x.Game).ToList();

                foreach (var game in games)
                {
                    ComboBoxGameCheatsFilterGame.Invoke((Action)(() =>
                    {
                        _ = ComboBoxGameCheatsFilterGame.Properties.Items.Add(game.Game);
                    }));
                }

                // List of values to ignore
                List<string> ignoreValues = new List<string> { "n/a", "-", "all regions", "all", "n", "a" };

                // Update regions
                foreach (var gameCheatItem in Database.GamesDataPS3.Games)
                {
                    foreach (string region in gameCheatItem.Region.Split(' ')
                        .Where(region => !ComboBoxGameCheatsFilterRegion.Properties.Items.Contains(region))
                        .Where(region => !ignoreValues.Exists(x => x.Equals(region, StringComparison.OrdinalIgnoreCase))))
                    {
                        ComboBoxGameCheatsFilterRegion.Invoke((Action)(() =>
                        {
                            _ = ComboBoxGameCheatsFilterRegion.Properties.Items.Add(region);
                        }));
                    }

                    // Update versions
                    foreach (string version in gameCheatItem.Version.Split('/')
                        .Where(version => !ComboBoxGameCheatsFilterVersion.Properties.Items.Contains(version))
                        .Where(version => !ignoreValues.Exists(x => x.Equals(version, StringComparison.OrdinalIgnoreCase))))
                    {
                        ComboBoxGameCheatsFilterVersion.Invoke((Action)(() =>
                        {
                            _ = ComboBoxGameCheatsFilterVersion.Properties.Items.Add(version);
                        }));
                    }
                }
            });
        ...
        /// </summary>
        /// 
        public async Task SearchGameCheats()
        {
            GridViewGameCheats.ShowLoadingPanel();
            ...
            {
                return Database.GamesDataPS3.Games.Where(x =>
                    x.Game.ContainsIgnoreCase(FilterGameCheatsName) &&
                    x.Region.ContainsIgnoreCase(FilterGameCheatsRegion) &&
                    x.Version.ContainsIgnoreCase(FilterGameCheatsVersion)).ToList();
            });

            foreach (var gameItem in filteredGames)
            {
            ...
            GridViewGameCheats.Columns[2].MinWidth = 166;
            GridViewGameCheats.Columns[2].MaxWidth = 166;

            GridViewGameCheats.Columns[3].MinWidth = 80;
            GridViewGameCheats.Columns[3].MaxWidth = 80;

            ComboBoxGameCheatsFilterRegion.SelectedIndexChanged -= ComboBoxGameCheatsFilterRegion_SelectedIndexChanged;
            ComboBoxGameCheatsFilterRegion.SelectedIndex = string.IsNullOrEmpty(FilterGameCheatsRegion)
                ? -1
                : ComboBoxGameCheatsFilterRegion.Properties.Items.IndexOf(FilterGameCheatsRegion);
            ComboBoxGameCheatsFilterRegion.SelectedIndexChanged += ComboBoxGameCheatsFilterRegion_SelectedIndexChanged;

            ComboBoxGameCheatsFilterVersion.SelectedIndexChanged -= ComboBoxGameCheatsFilterVersion_SelectedIndexChanged;
            ComboBoxGameCheatsFilterVersion.SelectedIndex = string.IsNullOrEmpty(FilterGameCheatsVersion)
                ? -1
                : ComboBoxGameCheatsFilterVersion.Properties.Items.IndexOf(FilterGameCheatsVersion);
            ComboBoxGameCheatsFilterVersion.SelectedIndexChanged += ComboBoxGameCheatsFilterVersion_SelectedIndexChanged;
        ...
        //    await Task.Run(() =>
        //    {
        //        var filteredGameCheats = Database.GameCheatsDataPS3.Where(x =>
        //            x.Game.ContainsIgnoreCase(FilterGameCheatsName) &&
        //            x.Region.ContainsIgnoreCase(FilterGameCheatsRegion) &&
        //            x.Version.ContainsIgnoreCase(FilterGameCheatsVersion)).ToList();

        //        foreach (var gameCheatItem in filteredGameCheats)
        //        {
        //            DataTableGameCheats.Rows.Add("", //gameCheatItem.FileName,
        //                                         gameCheatItem.Game,
        ...
        //    GridViewGameCheats.Columns[3].MinWidth = 112;
        //    GridViewGameCheats.Columns[3].MaxWidth = 112;

        //    //GridViewGameCheats.Columns[3].MinWidth = 78;
        //    //GridViewGameCheats.Columns[3].MaxWidth = 78;
        ...
        private void GridViewGameCheats_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            if (GridViewGameCheats.RowCount > 0)
            {
                if (e.FocusedRowHandle != -1)
                {
                    ...
                    SelectedGameItem = Database.GamesDataPS3.Games.First(x => x.File == (string)GridViewGameCheats.GetRowCellValue(e.FocusedRowHandle, GridViewGameCheats.Columns[0]));
                }
            }
        ...
        private void GridViewGameCheats_RowClick(object sender, RowClickEventArgs e)
        {
            if (e.RowHandle != -1)
            {
                ...
                SelectedGameItem = Database.GamesDataPS3.Games.First(x => x.File == (string)GridViewGameCheats.GetRowCellValue(e.RowHandle, GridViewGameCheats.Columns[0]));
            }
        ...
        private void GridViewGameCheats_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (info.InRow)
            {
                ...
                SelectedGameItem = Database.GamesDataPS3.Games.First(x => x.File == (string)GridViewGameCheats.GetRowCellValue(info.RowHandle, GridViewGameCheats.Columns[0]));
        ...
        private void LoadTrainersCategories()
        {
            ...
            _ = ComboBoxTrainersFilterGame.Properties.Items.Add($"<{ResourceLanguage.GetString("ALL_GAMES")}>");
            ...
            foreach (TrainerGameItem game in Database.TrainersX360.Library)
            {
                ...
                _ = ComboBoxTrainersFilterTitleId.Properties.Items.Add(game.TitleId);
            }
        ...
        private void ShowGameModDetails(Platform platform, int modId)
        {
            ModItemData modItemData = platform == Platform.PS3
                ? Database.GameModsPS3.GetModById(platform, modId)
                : Database.GameModsX360.GetModById(platform, modId);
        ...
        private void ShowHomebrewDetails(Platform platform, int modId)
        {
            ...
            else if (platform == Platform.PS4)
            {
                ...
                DialogExtensions.ShowItemAppDetailsDialog(this, appItemData);
                return;
            }
        ...
        /// </summary>
        /// <param name=platform"> Specifies the <see cref="Platform" /> </param>
        ...
        private void ImageLogo_ItemClick(object sender, SvgImageItemMouseEventArgs e)
        {
            DialogExtensions.ShowEasterEgg(this);
        }
        ...
        {
            _ = Process.Start(Urls.DiscordServer);
        }
        ...
        #region Logs, Connection Status and Enable Controls
        ...
        public void SetStatus(string status, Exception ex = null)
        {
            switch (ex)
            {
                ...
                default:
                    if (status != ResourceLanguage.GetString("STATUS_IDLING"))
                    {
                        StatusIdleTimer.Stop();
                        ...
                        //{
                        //    DialogExtensions.ShowReportIssueDialog(this, ex);
                        //}

                        _ = XtraMessageBox.Show(
                            this,
                            string.Format(ResourceLanguage.GetString("ERROR_OCCURRED"), ex.Message),
                            ResourceLanguage.GetString("ERROR"),
                            MessageBoxButtons.OK,
                            MessageBoxIcon.Error,
                            allowHtmlText: DefaultBoolean.True);
                    }
        ...
        private void StatusIdleTimer_Tick(object sender, EventArgs e)
        {
            SetStatus(ResourceLanguage.GetString("STATUS_IDLING"));
        }
        ...
        public void SaveApplicationSettings()
        {
            ...
            catch (Exception ex)
            {
                ...
                _ = XtraMessageBox.Show(this, string.Format("There is a problem saving the application settings data.\n\n{0}: {1}", ResourceLanguage.GetString("ERROR"), ex.Message), ResourceLanguage.GetString("ERROR"), MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
