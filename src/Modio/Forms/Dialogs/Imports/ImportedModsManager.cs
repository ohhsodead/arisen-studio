...
{
    public partial class ImportedModsManager : XtraForm
    {
        public ImportedModsManager()
        {
            InitializeComponent();
        }
        ...
        private void ImportedModsManager_Load(object sender, EventArgs e)
        {
        ...
        private void LoadBackupFiles()
        {
            ...
            GridMods.DataSource = dt;

            GridViewMods.Columns[0].Width = 200;
            GridViewMods.Columns[1].Width = 200;
            GridViewMods.Columns[2].Width = 125;
            GridViewMods.Columns[3].Width = 125;
        }

        private void GridViewBackupFiles_FocusedRowChanged(object sender, FocusedRowChangedEventArgs e)
        {
            switch (GridViewMods.SelectedRowsCount)
            {
                case > 0:
                    try
                    {
                        BackupFile backupFile = MainWindow.BackupFiles.BackupFiles[GridViewMods.FocusedRowHandle];
            ...
            ButtonEdit.Enabled = GridViewMods.SelectedRowsCount > 0;
            ButtonDelete.Enabled = GridViewMods.SelectedRowsCount > 0;
            ButtonDeleteAll.Enabled = GridViewMods.SelectedRowsCount > 0;
            ButtonBackupFile.Enabled = GridViewMods.SelectedRowsCount > 0 && MainWindow.IsConsoleConnected;
            ButtonRestoreFile.Enabled = GridViewMods.SelectedRowsCount > 0 && MainWindow.IsConsoleConnected;
        }

        private void ButtonEdit_Click(object sender, EventArgs e)
        {
            int backupFileIndex = GridViewMods.FocusedRowHandle;
        ...
        private void ButtonDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                MainWindow.BackupFiles.BackupFiles.RemoveAt(GridViewMods.FocusedRowHandle);
                LoadBackupFiles();
            }
        }
