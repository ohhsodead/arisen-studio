...
namespace ModioX.Forms.Dialogs.Details
{
    public partial class PluginDialog : XtraForm
    {
        ...
        private void PluginDialog_Load(object sender, EventArgs e)
        {
            ...
            if (MainWindow.Settings.FavoriteIds.Exists(x => x.Platform == PlatformPrefix.XBOX && x.Ids.Contains(ModItem.Id)))
            {
        ...
        private void ButtonFavorite_Click(object sender, EventArgs e)
        {
            if (MainWindow.Settings.FavoriteIds.Exists(x => x.Platform == PlatformPrefix.XBOX && x.Ids.Contains(ModItem.Id)))
            {
                MainWindow.Settings.RemoveFavorite(PlatformPrefix.XBOX, ModItem.Id);
                ButtonFavorite.Text = Language.GetString("Favorite");
                ButtonFavorite.SetControlTextWidth(28);
            }
            else
            {
                MainWindow.Settings.AddFavorite(PlatformPrefix.XBOX, ModItem.Id);
                ButtonFavorite.Text = Language.GetString("Unfavorite");
                ButtonFavorite.SetControlTextWidth(30);
            }
        }
