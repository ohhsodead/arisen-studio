...
namespace ModioX.Templates
{
    public static class GitHubTemplates
    {
        ...
        internal static void OpenRequestTemplate(string name, string type, string category, string author,
            string version, string systemType, string description, string links)
        {
            var requestTemplate = "issues/new?"
                                  + $"title={name} ({type})&"
                                  + "labels=mod request&"
                                  + "assignee=ohhsodead&"
